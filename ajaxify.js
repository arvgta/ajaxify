...
pO("scripts", { $s : false, cd0 : 0 }, { canonical: true, inline: true, inlinehints: false, inlineskip: "adsbygoogle", inlineappend: true, style: true }, function (o) {
    ...
    onetxt: function ($s) { //Add one inline JS script - pre-processing / validation
        ...
        if (!txt.iO(").ajaxify(") && ((inline && !_inlineskip(txt)) || $s.hasClass("ajaxy") || _inlinehints(txt))) _addtext(txt, t, $s.attr("id"), $s.attr("class")); //Check constraints
    },
    addtext: function (t, type, ID, Classes) { //Add one inline JS script - main function
        if(!t || !t.length) return; //Ensure input
        if(!type) type = "text/javascript"; //Validate type
        if(inlineappend || !type.iO("text/javascript")) try { return _apptext(t, type, ID, Classes); } catch (e) { }
    ...
    apptext: function (t, type, ID, Classes) {  $new = $("<script>")
    .attr("type", type)
    .text(t);
	
    if(ID) $new.attr("id", ID);
    if(Classes) $new.attr("class", Classes);
